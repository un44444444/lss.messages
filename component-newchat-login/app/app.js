 ...
 */
angular.module('mediaoperation.login', [
    'ui.router',//路由
    'ngAnimate',//动画
    'ngCookies',//cookie
    ...
    'module.virtual',
    'service.auth',
    'service.loginbyopenid',
    'service.getsecurityuserid',
    'service.mallofflineauthdict',
    'service.mallofflineshopsstaff',
    'service.mallofflineshopsstaffauth',
])
    .run(
        ['$http','$rootScope', '$state', '$stateParams', '$location','AUTH_EVENTS','POWER','AuthService','CacheFactory', function ($http,$rootScope, $state, $stateParams, $location,AUTH_EVENTS,POWER,AuthService,CacheFactory) {
            ...
            $rootScope.$on('$stateChangeStart', function (event, toState, toParams, fromState, fromParams) {
                ...
                /*首页不返回登录页*/
                if(toState.name == "virtual.login" && fromState.name == "home" && AuthService.isAuthenticated()){
                ...
                /*简单处理退出登录后返回到之前页面*/
                if(toState.name == "settings" && fromState.name == "virtual.login"){
                    loadCart(false, "loader");
                    event.preventDefault();
                    $state.go("virtual.login");
                }
